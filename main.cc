...
int main() {
  ...
  if (alpha_complex_from_points.create_complex(simplex)) {
    ...
    for (auto f_simplex : simplex.filtration_simplex_range()) {
      simplex.assign_key(f_simplex, cnt++);
    }
    for (int i = 0; i < simplex.num_simplices(); i++) {
      auto f_simplex = simplex.simplex(i);
      std::clog << "[ ";
    ...
    //   //   std::clog << vertex << " ";
    //   // }
    //   for (auto boundary : simplex.boundary_simplex_range(f_simplex)) {
    //     std::clog << simplex.key(boundary) << " ";
    //     // std::clog << "[ ";
    //     // for (auto vertex : simplex.simplex_vertex_range(boundary)) {
    //     //   std::clog << vertex << " ";
    //     // }
    //     // std::clog << "]";
    //   }
